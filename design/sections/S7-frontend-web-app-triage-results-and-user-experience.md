...
## Summary
A single-page frontend that lets an operator chat with the troubleshooting assistant, submitting an initial log or question and then refining with follow-ups. The UI renders a conversational response generated by the API (structured outputs are composed server-side). MVP goals:
- End-to-end user flow (submit → results) completes in < 60 seconds under normal load.
- Safe, readable rendering of assistant responses with evidence excerpts embedded in the conversation when available.
...
- API contract (MVP, per `services/api`)
  ...
  - Response (200): chat response with `request_id`, `timestamp`, `assistant_message`, `completion_state`, `next_question?`, `tool_calls[]`, `hypotheses[]`, `fix_steps[]`, `metadata`, and `conversation_id`.
  - POST /explain
    - Request: `{ request_id?, conversation_id?, response, incident_frame?, tool_results? }` (`response` is required; `tool_results` is a list of `{ id, output }`).
  - Response (200): same chat response schema as `/triage`.
  - GET /status
    - Response (200): `{ status, timestamp, dependencies[] }` where `dependencies` is a string list.
  - GET /metrics/summary
    - Response (200): `{ timestamp, api_latency_p50_ms, api_latency_p95_ms, source, sample_count? }`
  - GET /budget/status
    - Response (200): `{ usage_window, retry_after, token_limit, tokens_used, remaining_budget }`
...
- UI pages/components (MVP)
  - Single page app layout:
    - Chat thread with user messages and assistant replies
    - Single input box (first message triggers `/triage`, subsequent messages call `/explain`)
    - Results are displayed as conversational assistant messages with optional tool calls and final explanations/fix steps
  - Small, focused components:
    - ChatWindow, MessageBubble, ChatInput, LoadingState, ErrorBanner, MetricsPanel (session stats), OpsStatsPanel (ops stats).
...
- Citations are referenced via short evidence excerpts embedded in assistant replies.
...
5. Explain request lifecycle (0.5 day)
  - On follow-up, call POST /explain with `{ response, conversation_id, request_id? }`.
...
- Functional
  ...
  - Assistant messages include evidence excerpts when available.
